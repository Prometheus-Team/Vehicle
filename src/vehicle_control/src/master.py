...
class Master:
    ...
    def sendSensorData(self):
        if not self.sensorStreamConnected:
            try:
                self.sensorStreamSock.connect((self.IP, 8008))
        ...
        while True:
            ...
            self.sensorStreamSock.sendall(data)
    ...
    def routeCmd(self, cmd):
        cmdList = {
            ...
            'manualControl':['forward','backward','left','right','stop'],
    ...
    def getBestDestination(self, map):
        x,y = self.convertPointToGrid((self.curLoc[1].x, self.curLoc[1].y))
        nextDest = self.getNextDestinationSrv(map, x, y).nextPoint

        if nextDest and len(nextDest) > 0:
            return Location(nextDest[0], nextDest[1])
